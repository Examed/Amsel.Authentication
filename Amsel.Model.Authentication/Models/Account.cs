...
namespace Amsel.Model.Authentication.AccountModels
{
    ...
    public partial class Account : AccountInfo, IGuidEntity, ISynchronize<TenantEntity>
    {
        ...
        string GetRightsAccountString(IEnumerable<TenantRight> rights) {
            if((rights == null) || !rights.Any()) {
                return string.Empty;
            }

            StringBuilder builder = new StringBuilder();
            foreach(TenantRight item in from TenantRight item in rights where (item != null) && (item.Tenant != null)
                select item) {
