...
namespace Amsel.Ingress.Authentication.Ingress
{
    ...
    {
        [NotNull]
        private static readonly APIAddress AllTenantsURL = new APIAddress(AuthEndpointResources.ENDPOINT,
        ...
        {
            var tenant = await GetTenantByNameAsync(name);
            return tenant.Id;
        }
        ...
        {
            var nameValue = new KeyValuePair<string, string>("name", name);
        ...
        {
            var idValue = new KeyValuePair<string, string>("id", id.ToString());
